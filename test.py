...
@pytest.mark.parametrize(
    'xsl_file',
    ['article-works.xsl', 'article-fails.xsl']
)
def test_xsl_transform(xsl_file):
    html = TEXT.encode('ascii', 'xmlcharrefreplace')
    if PY == 3:
        html = html.decode('utf-8')
    parser = HTMLParser(recover=True, remove_blank_text=True, remove_comments=True)
    htmltree = etree.parse(StringIO(html), parser)
    resulttree = get_transform(xsl_file)(htmltree)
    result = etree.tostring(resulttree)
    if PY ==3:
        assert b'von denen Gregor Gysi' in result
    else:
        assert 'von denen Gregor Gysi' in result
